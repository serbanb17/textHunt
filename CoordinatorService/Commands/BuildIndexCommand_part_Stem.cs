...
namespace CoordinatorService.Commands
{
    ...
    {
        private void Stem(ILogger logger)
        {
            ...
            while (cursor.MoveNext())
            {
                ...
                foreach (var doc in currentBatch)
                {
                    ...
                    var reqBody = new ExecuteRequestModel() { Command = "stem", Arguments = arguments };
                    logger.LogInformation("{Class}::{Method} | Call worker for docId = {docId}", nameof(BuildIndexCommand), nameof(Stem), doc.Id);
                    _workersManager.DoWork("/command/execute", reqBody, null);
                }
            }

            logger.LogInformation("{Class}::{Method} | Waiting results from all workers", nameof(BuildIndexCommand), nameof(Stem));
